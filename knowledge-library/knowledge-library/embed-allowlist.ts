...
function isAllowedEmbedSrc(src: string): boolean {
  // legacy/exported embed snippets commonly use a protocol-relative src
  // ("//www.youtube.com/embed/x") instead of a full "https://" URL — treat it
  // as https, the only scheme this allowlist ever accepts, rather than
  // rejecting a legitimate YouTube/Spotify embed just because `new URL()`
  // has no scheme to resolve without a base.
