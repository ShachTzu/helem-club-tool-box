...
async function canManage(context: ResolverContext, knowledgeLibrary: KnowledgeLibraryNode): Promise<boolean> {
  ...
  if (user.contentAdmin) return true;
...
/**
 * granting/revoking editor access is restricted to full admins and to
 * users scoped in as content admins — an editor themselves can't add
 * other editors.
 */
function assertIsAdmin(context: ResolverContext): void {
  const user = context?.session?.user;
  if (user?.role !== 'admin' && !user?.contentAdmin) {
