...
it('accepts a protocol-relative YouTube src (legacy embed export format)', () => {
  const html = '<iframe src="//www.youtube.com/embed/GpndsBhe2mQ?rel=0"></iframe>';
  expect(validateEmbedHtml(html)).toEqual(html);
});

it('accepts a protocol-relative Spotify src', () => {
  const html = '<iframe src="//open.spotify.com/embed/episode/abc123"></iframe>';
  expect(validateEmbedHtml(html)).toEqual(html);
});
