 ...
 */
export type DomainFilterMockDomain = NonNullable<UseDomainsOptions['mockData']>[number];

/**
 * the 14 coping domains used across the Helem Club ecosystem, with sample
 * tagged-content counts (one domain is left with a zero count to demonstrate
 * the `onlyWithContent` behavior).
 */
export const MOCK_FILTER_DOMAINS: DomainFilterMockDomain[] = [
  {
    id: `anxiety`,
    slug: `anxiety`,
    name: `חרדה`,
